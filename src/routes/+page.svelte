<script lang="ts">
	...
	function transform() {
		...
		try {
			...
			if (xsltDoc.getElementsByTagName('parsererror').length) throw new Error('XSLT parse error');
		} catch {
...
<div class="flex flex-col h-screen bg-gray-950 text-gray-100 overflow-hidden">
	...
	<header class="flex items-center gap-3 px-4 py-2.5 bg-gray-900 border-b border-gray-800 shrink-0">
		...
		<div class="flex items-center gap-2 ml-2">
			<label for="sample-select" class="text-xs text-gray-400 uppercase tracking-wider"
				>Sample</label
			>
			<select
				id="sample-select"
		...
		<div class="ml-auto flex items-center gap-2">
			...
			<div class="flex items-center bg-gray-800 rounded-lg p-0.5 border border-gray-700">
				<button
					class="px-3 py-1 text-xs rounded-md transition-colors {outputMode === 'rendered'
						? 'bg-violet-600 text-white'
						: 'text-gray-400 hover:text-gray-200'}"
					onclick={() => (outputMode = 'rendered')}>Rendered</button
				>
				<button
					class="px-3 py-1 text-xs rounded-md transition-colors {outputMode === 'raw'
						? 'bg-violet-600 text-white'
						: 'text-gray-400 hover:text-gray-200'}"
					onclick={() => (outputMode = 'raw')}>Raw XML</button
				>
			</div>
			...
			>
				...
				>
					<path
						stroke-linecap="round"
						stroke-linejoin="round"
						d="m5.25 4.5 14.25 7.5-14.25 7.5V4.5z"
					/>
				</svg>
				Transform
			</button>
	...
	<div class="flex items-center px-4 py-1 bg-gray-900 border-b border-gray-800 shrink-0">
		<span
			class="text-xs {status.ok ? 'text-emerald-400' : 'text-red-400'} flex items-center gap-1.5"
		>
			<span
				class="inline-block w-1.5 h-1.5 rounded-full {status.ok ? 'bg-emerald-400' : 'bg-red-400'}"
			></span>
			{status.msg}
		</span>
	</div>
		...
		<section class="flex flex-col flex-1 min-w-0 bg-gray-950">
			<div class="flex items-center gap-2 px-3 py-2 bg-gray-900 border-b border-gray-800 shrink-0">
				...
				<span class="ml-auto text-xs text-gray-600"
					>{outputMode === 'rendered' ? 'Rendered HTML' : 'Raw output'}</span
				>
			</div>
			<div class="flex-1 min-h-0 overflow-hidden relative">
						...
						<div class="flex flex-col items-center justify-center h-full text-gray-600 gap-2">
							...
							>
								<path
									...
									stroke-linejoin="round"
									stroke-width="1.5"
									d="M2.25 15.75l5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909M3 21h18M3 3h18"
								/>
							</svg>
				...
				{:else}
					<pre
						class="absolute inset-0 overflow-auto p-4 text-xs text-green-300 font-mono leading-relaxed whitespace-pre-wrap bg-gray-950">{rawOutput ||
							'No output yet — run Transform first.'}</pre>
				{/if}
			</div>
		</section>
	</main>
